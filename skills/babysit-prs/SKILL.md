...
name: babysit-prs
description: >-
  Prepare open PRs in chinrw/stocks to merge-ready, automatically merge strict
  stacked PRs innermost-first, and report ready integration/root PRs for the
  user. Manual only: this workflow may push fix branches, create PRs, reply to
  and resolve review threads, and merge eligible stacked PRs.
argument-hint: "[PR number] [--snapshot-only] [--dry-run] [--merge-integration] [--max-waves N]"
...
# `/babysit-prs` — quality-first PR readiness and stacked-merge controller

This is a **Claude Code skill**, not a Hermes workflow. Run it in the main Claude
Code conversation with the skill-selected `best` model and `xhigh` controller
effort. Critical judgment and final-verification checkpoints run in dedicated
`inherit/max` subagents; bounded spec selection and ordinary composition checks
run in dedicated `inherit/xhigh` subagents.
...
3. Every PR is advanced as far as current external gates allow. When repository
   ...
   until that pass actually arrives. Use `/loop 30m /babysit-prs` for
...
Run locally. This workflow depends on the local Codex plugin, local worktrees,
and local credentials; do not use a cloud scheduled task as a substitute.

If context compaction occurs, or any write/merge rule becomes uncertain, reread
`${CLAUDE_SKILL_DIR}/SKILL.md` before the next remote write. Recheck at least
...
Parse `$ARGUMENTS` as an unordered set:
...
```bash
/babysit-prs --snapshot-only            # whole repo
/babysit-prs 403 --snapshot-only        # one PR and its descendants
```
...
The main skill conversation is the controller:

- model: `best` — Fable 5 when available, otherwise the latest Opus;
- effort: `xhigh` (or `max` when an environment override intentionally raises it);
- responsibilities: snapshots, DAG/state transitions, scheduling sibling
  agents, collision checks, deterministic gates, GitHub writes, and final merge
  authorization;
...
- Claude Code is older than `2.1.219`;
- `${CLAUDE_EFFORT}` is neither `xhigh` nor `max`;
...
### Claude judgment agents

Use the dedicated sibling agents installed with this skill. Never replace them
with the built-in `general-purpose` agent merely to keep the run moving.

| Agent type | Model | Effort | Responsibility |
|---|---|---:|---|
| `babysit-pr-spec-selector` | `inherit` | `xhigh` | Select exact specs/plans and compute `specHash` |
| `babysit-pr-finding-judge` | `inherit` | `max` | Adversarially classify Codex findings against code and spec |
| `babysit-pr-thread-judge` | `inherit` | `max` | Dispose unresolved bot/human review threads |
| `babysit-pr-verifier` | `inherit` | `max` | Independently accept or reject an implementation |
| `babysit-pr-composition-verifier` | `inherit` | `xhigh` | Verify an ordinary child-merge composition shortcut |
| `babysit-pr-critical-composition-verifier` | `inherit` | `max` | Verify a high-risk child-merge composition shortcut |

All six agents must be dispatchable by the `Agent` tool before their results can
authorize remote writes. Their files are expected under project or user
`.claude/agents/`. Claude Code v2.1.198+ has no interactive `/agents` wizard, so
do not use that command as a discovery check. Validate the files/frontmatter
mechanically; when the Agent tool exposes its available types, confirm the six
names there. Otherwise, let the first real dispatch be the operational check.
Do not launch six dummy max-effort agents merely to probe availability. If any
required dispatch reports an unknown or unavailable agent, produce a read-only
snapshot, mark the affected work `BLOCKED: missing-agent:<name>`, and do not
silently downgrade.

These agents:

- inherit the active `best` model;
- receive their effort from their own frontmatter;
- may read diffs/specs and use only the tools granted by their definitions;
- must not call `Agent`, dispatch Codex, or implement non-trivial fixes;
...
- Maximum live heavy sibling agents: `4`.
- Maximum simultaneous write-capable Codex tasks: `2`.
- Maximum writers targeting the same eventual base branch: `1`.
- Never allow Claude and Codex to edit overlapping files concurrently.
- Never launch two Codex writers over the same worktree or file set.
- Tests and builds run in the foreground. Never finish an agent turn while an
  untracked background gate is still running.
...
```text
CANONICAL_RUN_DIR=/home/chin39/Documents/play/stocks/.claude/babysit-prs/runs/${CLAUDE_SESSION_ID}/
```
...
Every agent must write its detailed result to the assigned artifact and return
only one line in this form:
...
- `maxRoundsPerInvocation` bounds a single Claude Code run only. It is never a
...
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/external_review.py" resolve-policy \
...
The 1800-second cooldown deliberately matches the `/loop 30m /babysit-prs`
cadence so long-running babysitting retries at the same rhythm without
spamming the PR.
...
(`codex-cli 0.145.0`, plugin `openai-codex/codex/1.0.6`):
...
```bash
node "${CLAUDE_SKILL_DIR}/scripts/probe-codex-capabilities.mjs" probe \
...
Detailed findings never enter the main Claude context. They are extracted and
...
A finding may enter the Claude finding-judge pipeline only when it carries
...
node "${CLAUDE_SKILL_DIR}/scripts/codex-job.mjs" collect \
  --receipt     "$ART/attempts/$ATTEMPT/launch-receipt.json" \
...
thread bodies. Thread bodies are fetched only inside a fresh judgment agent.
...
```bash
node "${CLAUDE_SKILL_DIR}/scripts/review-key.mjs" classify \
  --repo "$REPO" --pr "$PR" \
...
invocations, and Claude Code restarts, so it must carry the full retry state,
...
Launch a fresh `babysit-pr-spec-selector` agent. It reads the branch/title,
changed-file list, and candidate spec/plan names to select the relevant
documents and calculate `specHash`. It must not form findings yet.
...
### 10.4 Fresh Claude finding judge

Launch a fresh `babysit-pr-finding-judge` agent. Its definition uses
`model: inherit` and `effort: max`.
...
9. never implement code and never invoke `Agent`.
...
```bash
node "${CLAUDE_SKILL_DIR}/scripts/mutation-evidence.mjs" run \
...
A fresh `babysit-pr-finding-judge` assesses the empirical result. If mutation is
unsafe or cannot be isolated, classify the claim as unconfirmed rather than
inventing certainty.
...
When unresolved threads exist, launch a fresh `babysit-pr-thread-judge` agent.
Its definition uses `model: inherit` and `effort: max`. It must:
...
feedback. Exhaustion becomes `BLOCKED`; the main Claude controller must not
become an emergency code writer.
...
After every implementation round, launch a fresh `babysit-pr-verifier` agent.
Its definition uses `model: inherit` and `effort: max`. It must not edit code or
invoke `Agent`.
...
across invocations, and across Claude Code restarts until the bot actually
...
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/external_review.py" fresh-pass \
...
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/external_review.py" evaluate \
...
For ordinary non-critical changes, launch a fresh
`babysit-pr-composition-verifier` agent. Its definition uses `model: inherit`
and `effort: xhigh`. For security, auth/authz, data-integrity, financial,
concurrency, destructive-migration, or otherwise high-risk composition, launch
`babysit-pr-critical-composition-verifier` instead (`inherit/max`). The selected
verifier reads only:
...
`ESCALATE_MAX`; immediately run `babysit-pr-critical-composition-verifier` on
the same immutable evidence. Final composition verdicts are `ACCEPT`, `REVIEW`,
or `BLOCKED`.
...
- fresh Claude judgment or verification artifact accepts that key;
...
4. Dispatch independent read-only Codex reviews, at most four heavy agents live.
5. Dispatch the required named Claude judgment agents after their input
   artifacts are terminal.
6. Dispatch thread-disposition agents.
7. Batch confirmed fixes per parent.
8. Run Codex fix -> fresh Claude verifier sequentially.
...
The next `/babysit-prs` invocation reconstructs external-review state from the
GitHub status comment plus live comments and reactions, and posts the next
round as soon as it is due.

For continuous babysitting, run:

```text
/loop 30m /babysit-prs
```

This keeps retrying on the policy cooldown until the bot leaves a fresh pass
...
With `--snapshot-only` (section 1.1): observe and report only. No Codex task, no
Claude agent, no worktree, no test, no artifact beyond the four snapshot files,
and no GitHub write. Every action label is prefixed `🔎 SNAPSHOT:`.
...
When any PR is still `WAITING_CODEX`, remind the user that
`/loop 30m /babysit-prs` continues the retry loop automatically.
...
- This skill runs under Claude Code `best/xhigh` at the controller layer; it is
  not a Hermes profile.
...
- `--snapshot-only` dispatches no Codex task, no Claude agent, and no worktree.
